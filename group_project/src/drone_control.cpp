...
unsigned int panel_index = 0; // index of the current panel 
...
nav_msgs::Odometry drone_fix_vel;
...
void drone_fix_Vel_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    drone_fix_vel = *msg;
    drone.drone_lin_vel_x = drone_fix_vel.twist.twist.linear.x;
    drone.drone_lin_vel_y =  drone_fix_vel.twist.twist.linear.y;
    drone.drone_lin_vel_z =  drone_fix_vel.twist.twist.linear.z;
...
void evaluate_control_point(bool from_image)
{
   ...
   // cout <<"################# Panel Index = " << panel_index << endl;
    ...
    //Find equation parameters of the line r2 passing through vector Vy. Line r2 is perpendicular  to line r but passing throung origin --> coeff angolare è
    ...
    drone.x_target = abs(drone.control_x_coo);
...
/* -------------------------------------------------------------------------------------------------------------- */
    if (mission.navigation_iteration_count < 300) //cambiamento
    {
        ...
        Rotation_GF_to_BF_drone_pos(drone.drone_Yaw);
        cout << "WAIT IN P1 STARTING POSITION: " <<  endl;
        cout << "mission.navigation_iteration_count: " << mission.navigation_iteration_count <<  endl;
        drone.drone_vel_msg.linear.x = pid_x->position_control_knowing_velocity(check_x_b, drone.drone_x_b, 0,  drone.drone_lin_vel_x);
        drone.drone_vel_msg.linear.y = pid_y->position_control_knowing_velocity(check_y_b, drone.drone_y_b, 0,  drone.drone_lin_vel_y);
    }
...
/* ---------------------------------------------------------------------------------------------------------------------- */
   ...
   // cout << "++++++++++++++ actual yaw of the drone: "<< drone.drone_Yaw << endl; 
    ...
    if (mission.cartesian_distance_err < 1.5)
    {
        ...
        mission.state = 2;  
        mission.navigation_iteration_count = 0;                   //Cambio di array
...
void jump_structure_array(Structure *structure, PID *pid_x, PID *pid_y, PID *pid_z, PID *pid_yaw)
{
    ...
    float yaw_err = drone.drone_Yaw - drone.yaw_des;
    ...
    if (mission.cartesian_distance_err < 0.9 && yaw_err < 0.2)
    {
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    structure.init(N_structure, x_waypoints, y_waypoints, orientations);
    ...
    ros::Publisher vel = nh.advertise<geometry_msgs::Twist>("/cmd_vel", 10);
    ...
    ros::Subscriber vel_drone_sub = nh.subscribe("/ground_truth/state", 5, drone_fix_Vel_callback);
